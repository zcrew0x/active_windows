...
cls
title Windows Activation Tool - (C) 2023 Copyright KyleTran Inc.
set Unattended=
set _args=
set _elev=
set "_arg1=%~1"
...
set "ELine=echo: &call :DL_color "==== LOI ====" %Red% &echo:"
...
:DL_E_Admin
%ELine%
echo Please run this script by administrator
goto DL_Done
...
echo:
call :DL_color1 "     " %Black% &call :DL_color "Notification: your %winos% has been permanently activated." %Green%
call :DL_color1 "     " %Black% &call :DL_color "You do not need Activation." %Gray%
echo ___________________________________________________________________________________________
echo:
if defined Unattended goto DL_Done
echo ___________________________________________
echo:
choice /C:12 /N /M ">    [1] Active your windows  [2] Exit : "
...
echo Or missing files needed for Activation.
goto DL_Done
)
...
if defined Eval (
%ELine%
echo [%winos% ^| %winbuild%] Digital Copyright Activation isn't supported.
echo %winos%| findstr /I Server >nul && (
echo Unable to activate the Evaluation instance. Please install the full version of Windows.
) || (
echo Unable to activate the Evaluation instance. Please install the full version of Windows.
)
goto DL_Done
...
if "%osSKU%"=="" (
%ELine%
echo SKU is not correct. Canceling...
goto DL_Done
)
...
if "%osedition%"=="" (
%ELine%
echo [%winos% ^| %winbuild% ^| SKU:%osSKU%] Digital Copyright Activation isn't supported.
goto DL_Done
)
...
cls
echo Checing OS information                  [%winos% ^| %winbuild% ^| %arch%]
...
if defined PermAct (
call :DL_color "%winos% has been permanently activated." %Green%
goto :DL_Act_Cont
)

call :DL_color "Error %Error_Code_%" %Red%
...
if defined _miss (
%ELine%
echo File^(s^) missing from folder 'BIN'. Canceling...
echo:
echo !_miss!
)
exit /b
